...
<div class="row-fluid">
	...
	<ul class="nav nav-tabs" style="width: 100%">
					...
					<li <c:if test="${e.id==estateId}">class="active"</c:if>><a
						href="${ctx}/../rentAccounting?estateId=${e.id}">${e.name }</a></li>
	...
	<form  method="post"  action="rentAccounting" >
	<input type="hidden" name="estateId" value="${estateId }">
					...
					<li style="float: left;">
						<div class="row-fluid repaire-search" >
												...
												<c:if test="${condition.sta !=null}">
												缴费类型:
													<input <c:if test="${condition.sta==2}">checked="checked"</c:if> name="sta" type="radio" id="master"  value="2">签        约</input>
													<input <c:if test="${condition.sta==3}">checked="checked"</c:if> name="sta" type="radio" id="master"  value="3">在        租</input>
												</c:if>
												&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
											<!-- 状态:<input  name="src" type="radio" id="master" value="2">已结算</input>
												...
												<c:if test="${condition.sta==null}"> 
												核算状态: <input  name="src" type="radio" id="master" value="1">已核算</input>
												
													   <input  name="src" type="radio" id="master" value="2">未核算</input>														
												
													 
												</c:if>
												<c:if test="${condition.src !=null}">
												核算状态:<input <c:if test="${condition.src==1}">checked="checked"</c:if> name="src" type="radio" id="master" value="1">已核算</input>
													<input <c:if test="${condition.src==2}">checked="checked"</c:if> name="src" type="radio" id="master"  value="2">未核算</input>
					...
					<li style="float: left;">
						...
						<div class="row-fluid repaire-search" style="margin-bottom:0px;">							
												...
												<div class="span2 search-input" style="width:150px;margin-left: 50px;">
												...
												<select id="danyuanid" name="danyuanid" style="width:100px;">
														<option value="-1">请选择${danyuanid }</option>
			...
			<form id="Jform" action="balance2" method="post">
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>选择
				 </th><th>编号</th><th>物业</th><th>楼号</th><th>单元</th><th>房号</th><th>入住人</th><th>合同起始日期</th><th>合同截止日期</th><th>状态</th><th>核算状态</th><th>备注</th>
		<%--<th>角色</th> --%></tr></thead>
		<tbody>
	...
	<div>
			 ...
			 <c:forEach var="role" items="${user.rolesList}"><c:if test="${role.id==1 }">
					...
					<input type="button"  id="repaireApply" class="btn"  value="核算" onclick="Balance()"/>
						</c:if><c:if test="${role.id!=1 }"><c:forEach var="m" items="${role.moduleList }"><c:if test="${m.id==47}">
