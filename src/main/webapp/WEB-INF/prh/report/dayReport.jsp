...
<script type="text/javascript">
	...
	function checkMoney(){
		...
		$(".allMoneyTotal").text(allTotal.toFixed(2));
...
%>
	...
	<div class="row-fluid" style=" margin: 0px;padding: 0px;" >
		...
		<div class="row-fluid repaire-search" style="height: 28px; margin: 0px;padding: 0px;" >
			 <form method="post" id="searchForm" action="dayReport.do">
					...
					<div class="row-fluid">
						<div class="span2 search-input" style="width: 100%;height: 25px;margin-bottom: 10px;" >
							...
							<c:if test="${nowMonth==null }">
								<fmt:formatDate value="${now}" pattern="MM" var="nowMonth" />
							</c:if>
 							
 								<c:if test="${nowMonth==01||nowMonth==03||nowMonth==05||nowMonth==07||nowMonth==08||nowMonth==10||nowMonth==12 }">
 								<c:set var="daysNum" value="31"></c:set>
 							</c:if>
 							
 							<c:if test="${nowMonth==04||nowMonth==06||nowMonth==09||nowMonth==11}">
 								<c:set var="daysNum" value="30"></c:set>
 							</c:if>
 						
 							<c:if test="${nowMonth==02}" >
 								 <c:if test="${(nowYear%4==0||nowYear%400==0)&&(nowYear%100!=0)}">
 										<c:set var="daysNum" value="29"></c:set>
 								</c:if>
 								
 								
								<c:if test="${!( (nowYear%4==0||nowYear%400==0)&&(nowYear%100!=0) )}">
 										<c:set var="daysNum" value="28"></c:set>
 								</c:if> 
							...
							<select name="year" class="year" style="width: 70px;">
							  ...
							  月: 
							  <select name="month" class="month" style="width: 60px;">
								<c:forEach var="month" begin="1" end="12">
									<option value="${month}" <c:if test="${nowMonth==month }">selected="selected" </c:if>>${month}</option>
								</c:forEach>
							</select> &nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div class="main" id="main0" style=" width:90%;height:75%; margin: 0px;padding-right: 130px;overflow: scroll;">
			...
			class="table table-striped table-bordered table-condensed" style="border-collapse:collapse;width:80% !important;">	
				<tr >	
					...
					<td rowspan="3" colspan="2" style="width: 100px !important;">月累计</td>
					<td  colspan="${daysNum +3}" style="text-align: left;font-weight: bold;padding-left: 400px;">${title}</td>
						
				</tr>	
				<tr>
						<td colspan="10" style="height: 20px;!important;"></td>
						<td colspan="10" style="height: 20px;!important;"></td>
						<td colspan="${daysNum-20 }" style="height: 20px;!important;"></td>
				</tr>
										 
				<tr>
					
					<c:forEach var="day" begin="1" end="${daysNum}" >
					<td >${day }</td>
					</c:forEach>
				</tr>
...
<%-- 						<fmt:formatNumber type="number" value="${dayRents.get(day)-dayExitRents.get(day)}" pattern="0.00" maxFractionDigits="2"/> --%>
				...
				<tr>
					...
					<c:forEach var="day" begin="1" end="${daysNum }" >
					<td class="money rent2">${dayRents.get(day) }</td>
					</c:forEach>
				</tr>
				...
				<tr>
					...
					<c:forEach var="day" begin="1" end="${daysNum }" >
					<td class="money rent3">${dayExitRents.get(day) }</td>
					</c:forEach>
				</tr>
				<tr>
					<td>2.</td>
					...
					<td class="money desposit1">
						<fmt:formatNumber type="number" value="${dayDesposits.get(day)-dayExitDesposits.get(day) }" pattern="0.00" maxFractionDigits="2"/>
					</td>
					</c:forEach>
				</tr>
				...
				<tr>
					...
					<c:forEach var="day" begin="1" end="${daysNum }" >
					<td class="money desposit2">${dayDesposits.get(day) }</td>
					</c:forEach>
				</tr>
				...
				<tr>
					...
					<c:forEach var="day" begin="1" end="${daysNum }" >
					<td class="money desposit3">${dayExitDesposits.get(day)}</td>
					</c:forEach>
				</tr>
				...
				<tr>
					...
					<fmt:formatNumber type="number" value="${(dayRents.get(day)-dayExitRents.get(day))+(dayDesposits.get(day)-dayExitDesposits.get(day) ) }" pattern="0.00" maxFractionDigits="2"/>
		 	...
		 	<br/>
			<input  onclick="location.href='dayReportExport.do?estateId=${estateId}&year=${nowYear}&month=${nowMonth}'" value="导出报表" type="button" class="btn"> 
