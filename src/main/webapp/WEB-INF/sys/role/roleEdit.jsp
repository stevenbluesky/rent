...
.treeview li.lastCollapsable, .treeview li.lastExpandable { background-image: url(${ctxStatic}/jquery/images/treeview-default.gif);
...
<script type="text/javascript">
  $(function() {
		...
		texts.each(function(e, text) {
			...
			if ($(text).text().indexOf("()")!=-1) {
				$(text).html("<input type='hidden' value='0' class='level1CountHidden'>");
			}
		});
  ...
  function initCount(){
	...
	function chooseLevel(value){
		...
		var url="toRoleEdit.do?level1="+value+"&name="+name+"&modulesIds="+modulesIds+"&moduleNum="+moduleNum+"&level1Counts="+array+"&roleId="+roleId;
	...
	function choose(chk1){
		...
		var len= $(".mIds:checked").length;
		if (len==0) {
			radio.parent().find(".level1CountSpan").html("<input type='hidden' value='0' class='level1CountHidden'>");
			$(".leve1Radio:checked").next().val("");
			
			
		}else{
			radio.parent().find(".level1CountSpan").html("("+len+")<input type='hidden' value='"+len+"' class='level1CountHidden'>");
			$(".leve1Radio:checked").next().val(len);
...
<body style="overflow-y: scroll;overflow-x: hidden;">
	...
	<form id="myForm" method="post" >
		...
		<input type="hidden" name="isFirst" class="isFirst" value="${isFirst}">
		<input type="hidden" name="roleId" class="roleId" value="${role.id }">
		...
		<table class="addFloor" border="0" bordercolor="#a0c6e5">
			...
			<tr>
				...
				<td><input type="text" class="name" name="name" style="height: 30px;" value="${name==null?role.name:name}"></td>
			</tr>
			<tr>
				<td class="title">
				<!-- 已选择权限: -->
				...
				<c:if test="${isFirst==1}">
					<input type="hidden" name="modulesIds" value="${role.modules}" class="modulesIds">
				</c:if>
				<c:if test="${isFirst!=1}">
								...
								<td><input onchange="chooseLevel(this.value)" name="level1" id="level1Radio${m.id}" type="radio"
									...
									<span class="level1CountSpan" style="color: red;">
										...
										<c:if test="${isFirst==1}">
											...
											<c:set var="isAddHidden" value="0"/>
											
											<c:forEach var="rm" items="${role.moduleList }">
												<c:if test="${rm.id==m.id}">
												(<c:if test="${rm.rightsCountOfLevel1!=0 }">${rm.rightsCountOfLevel1 }</c:if>)
												
													<input type="hidden" value="${rm.rightsCountOfLevel1}" class="level1CountHidden">
													<c:set var="isAddHidden" value="1"/>
												</c:if>
											</c:forEach>
										
											<c:if test="${ isAddHidden==0}">
												<input type="hidden" value="0" class="level1CountHidden">
											</c:if>
  ...
  <div id="right" style="margin-left: 50px;">
   <ul class='filetree browser'>
   	  <c:forEach var="m" items="${level2Modules}" varStatus="status">
			...
			<input  
				<c:forEach var="selectedId" items="${modulesIdList}"><c:if test="${selectedId==m.id }">checked='checked'</c:if></c:forEach>
			...
			<c:forEach var="b" items="${m.btnModules}">
				...
				<input 
				<c:forEach var="selectedId" items="${modulesIdList}">
						<c:if test="${selectedId==b.id }">checked='checked'</c:if></c:forEach>
			...
			<!-- 换列 -->
			<c:if test="${fn:length(level2Modules)>4}">
				<c:set var="colNum" value="2"/>
			</c:if>
			<c:if test="${fn:length(level2Modules)<=4}">
				<c:set var="colNum" value="1"/>
			</c:if>
			<c:if test="${status.index==colNum }">
			</ul>
			</div>
			<div id="right" style="margin-left: 50px;">
				<ul  class='filetree browser'>
			...
			<tr style="text-align: center;">
				<td colspan="2"><br />
				<br /> <input class="btn" type="button" value="修改"
