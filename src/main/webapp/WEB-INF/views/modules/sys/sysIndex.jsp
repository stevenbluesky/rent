...
<body>
 ...
 %> <fmt:formatDate value="${now }" pattern="yyyy-MM-dd" />
				...
				<div class="accordion" id="menu-${parentId }" 
					style="position: absolute; top: 71px; width: 165px; left: 0;">
					...
					<c:forEach items="${menuList}" var="menu" varStatus="idxStatus">
						...
						</c:if><c:if test="${role.id!=1 }"><c:forEach var="m" items="${role.moduleList }"><c:if test="${m.menuId==menu2.id}">
					...
					<li class='menu-li'><a data-href=".menu3-${menu2.id}"
						...
						</c:if><c:if test="${role.id!=1 }"><c:forEach var="m" items="${role.moduleList }"><c:if test="${m.menuId==menu.id}">
